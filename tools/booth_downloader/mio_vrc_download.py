...
MANIFEST_CACHE_BUST_PARAM = "_mio_update_check"
TRUSTED_DOWNLOAD_HOSTS = {
    "78hejiu.top",
    "download.78hejiu.top",
}
TRUSTED_GITHUB_OWNER = "cokoiya"
TRUSTED_GITHUB_REPO = "miovrc_translator"
TRUSTED_GITHUB_RELEASE_PREFIX = (
    f"/{TRUSTED_GITHUB_OWNER}/{TRUSTED_GITHUB_REPO}/releases/download/"
)
...
def _trusted_download_host(hostname: str | None, *, allow_release_asset_redirect: bool = False) -> bool:
    ...
    if not host:
        return False
    if host in TRUSTED_DOWNLOAD_HOSTS or host == "github.com":
        return True
...
def is_trusted_download_url(url: str, *, allow_release_asset_redirect: bool = False) -> bool:
    ...
    if not _trusted_download_host(
        ...
        return False
    if str(parsed.hostname or "").strip().lower() == "github.com":
        return parsed.path.lower().startswith(TRUSTED_GITHUB_RELEASE_PREFIX)
    return True


def manifest_filename(value: str, field_name: str, suffixes: tuple[str, ...]) -> str:
    raw = str(value or "").strip()
    if not raw:
        raise RuntimeError(f"manifest {field_name} is missing")
    normalized = raw.replace("\\", "/")
    filename = PurePosixPath(normalized).name
    if filename != raw or filename in {"", ".", ".."} or ":" in filename:
        raise RuntimeError(f"manifest {field_name} is invalid")
    lowered = filename.lower()
    if suffixes and not lowered.endswith(suffixes):
        raise RuntimeError(f"manifest {field_name} has an unsupported file type")
    return filename
...
def parse_optional_size(value: object, field_name: str) -> int | None:
    ...
    if size_bytes > MAX_DOWNLOAD_BYTES:
        raise RuntimeError(f"manifest {field_name} is too large")
    return size_bytes


def validate_required_sha256(value: str, field_name: str) -> str:
...
def request_url(url: str, timeout: int = 20):
    if not is_trusted_download_url(url):
        raise RuntimeError("download URL is not trusted")
...
def manifest_request_url(url: str, *, timestamp_ms: int | None = None) -> str:
    parsed = urllib.parse.urlparse(str(url or "").strip())
    query = [
        (key, value)
        for key, value in urllib.parse.parse_qsl(parsed.query, keep_blank_values=True)
        if key != MANIFEST_CACHE_BUST_PARAM
    ]
    if timestamp_ms is None:
        timestamp_ms = int(time.time() * 1000)
    query.append((MANIFEST_CACHE_BUST_PARAM, str(timestamp_ms)))
    return urllib.parse.urlunparse(parsed._replace(query=urllib.parse.urlencode(query)))


def verify_public_manifest(payload: dict[str, object]) -> None:
    try:
        ...
        raise RuntimeError(f"manifest signature check failed: {exc}") from exc
    if REQUIRE_UPDATE_MANIFEST_SIGNATURE and not verified:
        raise RuntimeError("manifest signature is required")


def load_manifest(manifest_url: str) -> InstallerManifest:
    try:
        with request_url(manifest_request_url(manifest_url)) as response:
    ...
    verify_public_manifest(payload)
    ...
    if not is_trusted_download_url(installer_url):
        raise RuntimeError("manifest installer_url is not trusted")
    sha256_key = "full_sha256" if use_full_installer else "sha256"
    sha256 = validate_required_sha256(str(payload.get(sha256_key) or ""), sha256_key)
    ...
    if model_package_url and not is_trusted_download_url(model_package_url):
        raise RuntimeError("manifest model_package_url is not trusted")
    model_sha256 = (
        validate_required_sha256(str(payload.get("model_sha256") or ""), "model_sha256")
        if model_package_url
        else ""
    )
    homepage_url = str(payload.get("homepage_url") or DEFAULT_HOMEPAGE_URL).strip() or DEFAULT_HOMEPAGE_URL
    app_exe = manifest_filename(
        str(payload.get("app_exe") or DEFAULT_APP_EXE).strip() or DEFAULT_APP_EXE,
        "app_exe",
        (".exe",),
    )
    ...
    installer_name = manifest_filename(installer_name, installer_name_key, (".exe",))
    ...
    if model_package_url:
        model_package_name = manifest_filename(
            model_package_name,
            "model_package_name",
            (".zip",),
        )
...
def download_installer(
    ...
    try:
        with request_url(installer_url, timeout=60) as response, partial_path.open("wb") as handle:
            if not is_trusted_download_url(response.geturl(), allow_release_asset_redirect=True):
