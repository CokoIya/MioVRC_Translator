...
from src.version import UPDATE_MANIFEST_PUBLIC_KEY_ID
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--notes-ja", default="", help="Optional Japanese release notes.")
    parser.add_argument("--notes-en", default="", help="Optional English release notes.")
    parser.add_argument(
        "--signing-seed-file",
        default=os.environ.get("MIO_UPDATE_MANIFEST_SEED_FILE", ""),
        help="Optional Ed25519 private seed file. Defaults to MIO_UPDATE_MANIFEST_SEED_FILE.",
    )
    parser.add_argument(
        "--signature-key-id",
        default=os.environ.get("MIO_UPDATE_MANIFEST_KEY_ID", UPDATE_MANIFEST_PUBLIC_KEY_ID),
        help="Signature key id written to the manifest.",
    )
    parser.add_argument(
        "--published-at",
        default="",
...
def main() -> int:
    ...
    if notes:
        payload["notes"] = notes
    notes_ja = str(args.notes_ja).strip()
    if notes_ja:
        payload["notes_ja"] = notes_ja
    notes_en = str(args.notes_en).strip()
    if notes_en:
        payload["notes_en"] = notes_en
    signing_seed_file = str(args.signing_seed_file).strip()
    if signing_seed_file:
        seed = Path(signing_seed_file).read_text(encoding="utf-8").strip().split()[0]
        payload[SIGNATURE_ALGORITHM_FIELD] = SIGNATURE_ALGORITHM
        signature_key_id = str(args.signature_key_id).strip()
        if signature_key_id:
            payload[SIGNATURE_KEY_ID_FIELD] = signature_key_id
