...
_COPY = {
    "zh-CN": {
        ...
        "overlay_body": "把别人说的话翻译后显示在头显里的字幕板（需要 SteamVR）和电脑上的悬浮字幕里；同时打开双手取景翻译：两手按住握把在面前框住文字，保持不动就能看到译文。",
    ...
    "en": {
        ...
        "overlay_body": "Show translated captions of what others say on the headset subtitle board (needs SteamVR) and in the desktop floating window, and switch on the two-hand frame: hold both grips, frame some text and keep still to see it translated.",
    ...
    "ja": {
        ...
        "overlay_title": "VR 字幕",
        "overlay_body": "相手の発話の翻訳を、ヘッドセット内の字幕ボード（SteamVR が必要）とデスクトップのフローティング字幕に表示し、両手フレーム翻訳もオンにします：両手のグリップを握って文字を枠で囲み、静止すると訳文が表示されます。",
    ...
    "ru": {
        ...
        "overlay_body": "Показывать перевод речи других игроков на панели субтитров в шлеме (нужен SteamVR) и в плавающем окне на ПК, а также включить рамку руками: зажмите оба грипа, возьмите текст в рамку и замрите — появится перевод.",
    ...
    "ko": {
        ...
        "overlay_title": "VR 자막",
        "overlay_body": "다른 사람의 말을 번역해 헤드셋 자막 보드(SteamVR 필요)와 PC 플로팅 창에 표시하고, 양손 프레임 번역도 켭니다: 양손 그립을 쥐고 글자를 프레임으로 감싼 채 가만히 있으면 번역이 보입니다.",
