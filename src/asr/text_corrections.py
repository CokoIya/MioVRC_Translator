...
def _fresh_user_dictionary() -> dict[str, Any]:
    return {
        "version": USER_DICTIONARY_TEMPLATE["version"],
        "source": USER_DICTIONARY_TEMPLATE["source"],
        "description": USER_DICTIONARY_TEMPLATE["description"],
        "entries": [],
    }


def _coerce_pattern_list(raw: Any, *, split_text: bool = False) -> list[str]:
    if raw is None:
        values: list[Any] = []
    elif split_text and isinstance(raw, str):
        values = re.split(r"[\n,，;；、]+", raw)
...
def _coerce_entry_patterns(entry: dict[str, Any]) -> list[str]:
    patterns = _coerce_pattern_list(entry.get("patterns"), split_text=False)
    if patterns:
        return patterns
    return _coerce_pattern_list(
        entry.get("pattern") or entry.get("match"),
        split_text=False,
    )


def _normalize_rule_mode(value: Any) -> str:
    mode = str(value or "substring").strip().lower()
    return mode if mode in {"substring", "exact", "word"} else "substring"
...
def _load_user_dictionary_payload(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"User dictionary is invalid JSON: {exc}") from exc

    if not isinstance(payload, dict):
        raise RuntimeError("User dictionary must be a JSON object")
    return payload


def upsert_user_dictionary_entry(
    replacement: str,
    wrong_terms: str | list[str] | tuple[str, ...],
) -> dict[str, Any]:
    replacement_text = str(replacement or "").strip()
    if not replacement_text:
        raise ValueError("replacement must not be empty")

    new_patterns = _coerce_pattern_list(wrong_terms, split_text=True)
    if not new_patterns:
        raise ValueError("wrong_terms must not be empty")

    ensure_user_dictionary()
    path = user_dictionary_path()
    if path.exists():
        payload = _load_user_dictionary_payload(path)
    else:
        payload = _fresh_user_dictionary()
    ...
    for index, raw_entry in enumerate(entries):
        if not isinstance(raw_entry, dict):
            continue
        raw_replacement = str(
            raw_entry.get("replacement") or raw_entry.get("replace") or ""
        ).strip()
        if raw_replacement == replacement_text:
            matching_indices.append(index)

    duplicate_patterns: list[str] = []
    if matching_indices:
        entry = entries[matching_indices[0]]
        for index in matching_indices[1:]:
            duplicate_entry = entries[index]
            if isinstance(duplicate_entry, dict):
                duplicate_patterns.extend(_coerce_entry_patterns(duplicate_entry))
    ...
    existing_patterns = _coerce_entry_patterns(entry)
    merged_patterns = _coerce_pattern_list(
        [*existing_patterns, *duplicate_patterns, *new_patterns],
        split_text=False,
    )
    entry["replacement"] = replacement_text
    entry["patterns"] = merged_patterns
    entry["mode"] = _normalize_rule_mode(entry.get("mode"))
...
def update_official_dictionary(config: dict | None = None) -> dict[str, Any]:
    ...
    if not re.fullmatch(r"[0-9a-f]{64}", expected_sha256 or ""):
        raise RuntimeError("Dictionary manifest is missing a valid SHA256")
